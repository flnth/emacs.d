...
(defun hs-org/hideshow (&optional key)
  ...
  (let* ((last-point (point))
    ...
    (when (and (equal last-point (point))
               (not mark-active))
      (outline-toggle-children)  ; was hs-toggle-hiding
...
;;       (hs-life-goes-on (if (hs-already-hidden-p) (progn (hs-show-block) (when
;;        hs-org/last-point (goto-char hs-org/last-point))) (setq
;;        hs-org/last-point (point)) (hs-hide-block)))
      )))
  ...
  (let* ((last-point (point))
    ...
    (when (equal last-point (point))
      (if hs-org/hide-show-all-next
          (show-all)   ; was hs-show-all
          (hide-body-recenter))  ; was hs-hide-all
